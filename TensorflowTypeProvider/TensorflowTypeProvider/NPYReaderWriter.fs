...
type NPYDType = 
    ...
    with 
        ...
        member this.TryToType() : Type option =
            match this with
            | Bool      -> typeof<bool> |> Some
            | Byte      -> typeof<byte> |> Some
            | UInt8     -> typeof<uint8> |> Some
            | UInt16    -> typeof<uint16> |> Some
            | UInt32    -> typeof<uint32> |> Some
            | UInt64    -> typeof<uint64> |> Some
            | Int8      -> typeof<int8> |> Some
            | Int16     -> typeof<int16> |> Some
            | Int32     -> typeof<int32> |> Some
            | Int64     -> typeof<int64> |> Some
            | Float16   -> None //failwith "unsupported"
            | Float32   -> typeof<single> |> Some
            | Float64   -> typeof<double> |> Some
...
type NPYDescription = 
    ...
    with
        ...
        override this.ToString() =
            sprintf "%O (%O)" this.npyDType (this.shape |> Array.map string |> String.concat ",")
...
let readNumpy(bytes:byte[]) : Result<(NPYDescription*Array),string> =
    let convertToArray(bytes:byte[],offset:int,shape:int[],dtype:NPYDType) : Array =
        ...
        | NPYDType.Int64 ->
            ...
            returnArray :> Array
        | NPYDType.Float16 -> 
            failwith "unsuported at this time"
            //let returnArray = Array.zeroCreate<uint16> length
            //Buffer.BlockCopy(bytes,offset,returnArray,0,length * dtype.ByteWidth)
...
let writeHeader(dtype : NPYDType,shape : int[]) =
    let typeDesc = sprintf "%c%s" (if dtype.ByteWidth = 1 then '|' else '<') (dtype.ToNPYString())
    sprintf "{'descr': '%s', 'fortran_order': False, 'shape': (%s), }" 
...
let writeArrayToNumpy(arr:Array,shape:int[]) : byte[] = 
    ...
    let preambleLen = 10
    let header = writeHeader(dtype,shape)
